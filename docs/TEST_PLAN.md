...
**Not proven, and the list is short but decisive.** No audio was decoded, mixed, or played. Neither
platform's scheduled start has run on a phone, an emulator or a simulator. No `AVAudioUnitVarispeed`
and no `ExoPlayer.setPlaybackParameters` has ever changed a real rate. **No alignment figure of any
...
| **5 Sync playback** | + `session-clock/` (29) + `ordering/` (70) + `drift/` (27 rows, 13 sequences) + `queue/` (22 scenarios, 4 snapshots, 2 cap rows) + `playback-messages/` (94) + `queue-messages/` (85), all passing both platforms — **done, §2aa**; + the coordinator suites (Android `app.sync` 32 cases; iOS `RideLinkPlatform` 31), the pre-authentication Phase 5 refusal over **real TLS** on both platforms, and a two-peer integration on each: iOS over a real authenticated TLS connection with the real `ClockSync` estimator (mapped session start error **47 us**, software scheduling only), Android in-process on clocks 7.5 s apart so the offset arithmetic is genuinely exercised — **done, §2aa** | player scheduling — *not run: neither platform's scheduled-start path has executed on a device or emulator, and the Android WebRTC-style `Context` problem does not apply here, so this is a gap of time rather than of possibility* | **S-01…S-12 pending** (§5.2); I-09, I-12 pending | — | — | **drift p95 pending (S-09); no alignment figure exists (S-03)** |
