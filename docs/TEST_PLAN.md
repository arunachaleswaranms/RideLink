...
| 4 Shared library | manifest-paging (13) + manifest-paging-errors (21) + manifest (13) + manifest-messages (35) + transfer-messages (44) + transfer-fsm (47) + bulk-framing (15) vectors, all passing both platforms — **done, §2u**; + closure-audit unit suites (`OperationFenceTest[s]`, `BulkTokenTableTest[s]`, `BulkTransportManagerTest`/`TransferManagerTests` frame-order/reentrancy/`cancelActive` additions, `TransferCacheRepositoryTest[s]` `verifiedHashes`, `ManifestEntryRowIdTests`), all passing both platforms and stress-run 10–20× clean — **done, §2v** | real-loopback-TLS bulk transfer (multi-chunk, SPKI-pin-then-token gated, both platforms) + manifest paginate/sync round-trip — **done, §2u**; a real Android emulator and a real iOS simulator both smoke-checked (install/launch, no crash, Shared Library UI correctly gated) — **done, re-confirmed §2v** | I-10, I-11, I-23, I-24 | — | — | ADR-023 (bulk session binding), **now Amendment A1** — closure audit confirmed and fixed 16 of 18 suspected integration/lifecycle gaps (live auth-generation reads, session-bound bulk-listener/operation lifecycle via a new `OperationFence` primitive, real transfer cancellation, corrected iOS actor-reentrancy claim, bulk frame-order validation, cache-only playback wiring, `TRANSFER_CANCEL` handling, sender-side size guard, a real manifest revision counter, session-epoch-scoped manifest dispatch — full list in ADR-023 Amendment A1 and `STATUS.md` §2v); one finding (same-size local-file replacement) is a documented, unchanged, no-integrity-consequence limitation; one (app-layer transfer control flow being ad-hoc rather than reducer-driven) remains **disclosed tech-debt, not a blocker**, closed instead by the `OperationFence` guard. **A dedicated `SharedLibraryCoordinator`-level real-TLS integration test was not added on either platform** — module-boundary/test-harness limits, disclosed in `STATUS.md` §2v, recommended as follow-up. **No real phone-to-phone transfer, no real Wi-Fi/hotspot topology, no 50 MB/5 000-entry real-device figure — all four I-rows pending.** |
