...
**Updated:** 8 September 2026 (Phase 5 — synchronized playback, twenty-fourth session — see §2aa)
...
music) now has its software half**: Phase 5 is implementation-complete on both platforms with its
real-device gate open.
**Current phase:** Phase 5 — synchronized playback (this session, §2aa). Clock-scheduled `PLAY`/
`PAUSE`/`RESUME`/`SEEK`/`NEXT`/`PREVIOUS`, a replicated shared queue, drift measurement and the
ADR-004 correction ladder, all on top of the Phase 1a clock layer and the Phase 3 player. Six new
shared vector sets, each generated by an independent third transcription. **Phase 6 (intercom/music
coexistence) and Phase 7 (Ride Mode) are untouched.**
**Phase 5 status: IMPLEMENTATION COMPLETE — REAL-DEVICE SYNCHRONIZED-PLAYBACK GATE PENDING.**
Deliberately *not* "software closure complete": Phase 5 has not been closure-audited even once, and
the standing lesson from Phase 4's five audits — that on this codebase "CI-green" and "correct" are
different claims — applies to this phase exactly as it did to that one. An independent audit is the
expected next step.
...
- **Nothing ran on a phone.** No audio was decoded, mixed or played anywhere in this phase.
- **Neither platform's scheduled start has run on a device, an emulator or a simulator.** Unlike the Android WebRTC gap (§4 problem 22), nothing prevents it — this is a gap of time, not of possibility, and it is the first thing an audit of this phase should close.
- **No `AVAudioUnitVarispeed` and no `ExoPlayer.setPlaybackParameters` has ever changed a real rate.**
...
| 41 | **Phase 5's scheduled start has never run on a device, an emulator or a simulator.** Both platforms' scheduling is proven against a *virtual* clock and, on iOS, over a real loopback TLS connection with the real estimator — but `ExoPlayer` and `AVAudioEngine` have never actually been told to start at a deadline, and neither `AVAudioUnitVarispeed` nor `setPlaybackParameters` has ever changed a real rate. **Unlike problem 22, nothing prevents this** — the emulator and simulator both exist and both already run Phase 3 media tests | Medium | The first thing a Phase 5 closure audit should close. TEST_PLAN §5.2's S-01…S-12 are the device gate beyond it |
...
**Phase 5 — synchronized playback. IMPLEMENTATION COMPLETE — REAL-DEVICE SYNCHRONIZED-PLAYBACK GATE
PENDING (§2aa).** Every laptop-runnable gate is green on both platforms: six new shared vector sets
passing identically on the first run against independently generated expectations, the coordinator
suites on both platforms, the Phase 5 pre-authentication refusal proven over **real TLS** with a real
unpaired first meeting, and two complementary two-peer integrations (iOS over a real authenticated
TLS connection with the real clock estimator; Android in-process on clocks 7.5 s apart).

**"Implementation complete" is a narrower claim than Phase 4's "software closure complete", and
deliberately so.** Phase 5 has **not been closure-audited even once**. Phase 4 was audited five times
and every pass found real integration or lifecycle defects in code that was already CI-green — in
session lifetime, cancellation ownership and platform I/O contracts rather than in the wire format or
the pure domain layer. There is no reason to believe Phase 5 is different, and two of its own bugs
this session were found by tests rather than by review. **An independent Phase 5 closure audit is the
expected next step**, and it should start with §4 problem 41 (nothing scheduled has ever run on a
device or emulator) and with the coordinator lifetimes, which is where Phase 4's audits kept finding
things.

**Immediately actionable next steps, in order:**

1. **Audit Phase 5.** Same discipline as ADR-023 Amendments A1–A5: re-derive the findings from the current production code rather than from this file, and look hardest at session/epoch lifetime across suspensions, at what a link loss leaves behind, and at the two places Phase 5 touches Phase 4 and Phase 2b state.
2. **Run the scheduled-start path on the emulator and the simulator** (§4 problem 41). Both already run Phase 3 media tests; there is no `Context`-shaped obstacle here.
...
5. **Run the Phase 1b gate**: I-02, I-03, I-04, I-16, I-19, I-20, I-21. I-02 is still the single most important test in the repository.
6. **Run the Phase 2a voice gate**: TEST_PLAN §5.1's V-01…V-11.
7. **Run the Phase 2b intercom gate**: A-10, IA-01…IA-03, AF-01/AF-03/AF-05, V-05/V-06/V-09.
8. **Run the Phase 4 gate**: I-10, I-11, I-23, I-24 — a transfer over a real network path, not loopback.
9. **Run the Phase 5 gate — TEST_PLAN §5.2's S-01…S-12.** S-03 is the one that matters most: **the first alignment figure this project will ever have.** Until it exists, the <100 ms product target and the <50 ms stretch target are unmeasured, and the 47 µs the local two-peer test reports is a *software scheduling* number in one process that says nothing about a speaker.
10. **Run the audio-hardware gate**: A-12…A-15. **The product's viability sits here.**
11. **Record every result** — pass, fail and measured numbers — in `docs/test-results/`.
12. **Then the recorded tech debt**: extract a `PairingController` from `ControlSessionManager` (§4 problem 18), close the remaining `SessionCoordinator` testability gap (§4 problem 20 — Phase 5 narrowed it by putting iOS's sync coordinator in `RideLinkPlatform`, but the app-target coordinators are still untested there), the manual `host:port`/QR fallback for blocked mDNS (§4 problem 19), and the cross-platform integral-float divergence (§4 problem 40) in a change that is *only* that.

**Gate for Phase 5:** S-01…S-12 recorded with S-03's and S-09's numbers written down; `session-clock/`,
`ordering/`, `drift/`, `queue/`, `playback-messages/` and `queue-messages/` passing on both platforms
(✅ **already true**); the Phase 5 pre-authentication refusal holding (✅ **already true, over real TLS
on both platforms**).

**Phase 6 (intercom/music coexistence) has not started and must not start** before the Phase 2b
device gate has numbers in it — the reasoning in the amendment below is unchanged, and Phase 6 is
precisely the coexistence work that reasoning is about. **Phase 7 (Ride Mode + resilience) has not
started.**

> **Do not treat Phase 5 as complete because the laptop tests are green.** They prove the tables, the
> codecs, the ordering, the session binding and the wiring — and two of the things they caught this
> session were real defects, so they are worth having. What they do not touch: any decoder, any
> mixer, any speaker, any Bluetooth endpoint, any lock screen on a real phone, and **any alignment
> measurement whatsoever**. The single number this phase produced (47 µs) is two `Task.sleep`
> wake-ups in one process on one machine.
