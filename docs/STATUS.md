...
unmodified production before fixing, on both platforms. Measured this pass: **1 036 Android tests**
across `core`/`network`/`app`/`audio`/`data` and **596 + 343 iOS tests** (`RideLinkPlatform` +
...
| 79 | **FIXED 20 September 2026 (found by problem 76's own regression).** A follower that is desynchronised **and** holds a deferred reconciliation re-sent `STATE_REQUEST` on every `SyncPlaybackDiagnostics` emission: `ingressDesynchronized` stays true until the retained snapshot applies, and `StateResyncGate` cannot refuse the repeat because the wire request was legitimately completed by that very snapshot. Unbounded — it exhausted the JVM heap in the regression before it was closed; on a real socket it is a flood on the control plane | ~~High~~ Fixed | A reconciliation already accepted and retained for this generation suppresses the retrigger. The obligation itself is the guard — not a timer, not a count — because a second copy of the same authoritative state cannot say anything the retained one does not. Mirrored on both platforms |
