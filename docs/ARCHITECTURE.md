...
3. Only a **deliberate end** may release the audio session and stop the foreground service, and there are exactly two: entering `ENDING`, and the user's explicit retry out of `DISCONNECTED` ([ADR-026](DECISIONS/ADR-026-session-lifecycle-teardown-and-restart.md)). A link blip (`RECONNECTING`) is neither — that is what the rule exists for, because §6.4 gives no second chance to reopen a microphone once the screen is locked. `SessionFsm` decides which ends are deliberate, not a coordinator, and `vectors/session-fsm/` pins the effect's presence **and its absence**.
...
9. **A session may enter `IDLE` only when it is terminal** ([ADR-026](DECISIONS/ADR-026-session-lifecycle-teardown-and-restart.md)). `TeardownComplete` — the one event that opens `ENDING -> IDLE`, and therefore the door a successor session walks through — may be emitted only after every effect owned by the ending session has **completed**: capture released and awaited, every continuation the session started cancelled **and joined**, and `ControlSessionManager.shutdown()` awaited rather than launched. No continuation owned by the retired session may mutate coordinator, relay, voice, playback, discovery, foreground-service or control-session state after it. There is exactly one teardown owner per platform (`SessionCoordinator.retireSession`, over `SessionTeardownOwner`), it captures everything the ending session owns **synchronously** before its first suspension, and a successor joins its job before touching anything shared. Cancellation is a request; joining is the proof.

10. **A relay sink belongs to whoever installed it, and only its installer may remove it.** `voice` and `audioState` are per authenticated session and detached by `SessionCoordinator`; `manifest`, `transfer` and `playback` are installed once per process by coordinators that deliberately outlive a control-session boundary, and are guarded by ADR-023 §3's / ADR-025's per-frame generation rather than by detachment.
...
| Voice | `stasel/WebRTC` **`152.0.0`** exact (Chromium M152), BSD-3-Clause | Maintained SPM distribution of *unmodified* upstream WebRTC. An SPM `binaryTarget`, so its SHA-256 is verified at resolve time — checked independently against the published release. `NSPrivacyTracking: false`, no collected data types, no tracking domains, no upload endpoint. Its XCFramework carries a **macOS** slice, which is what lets `swift test` run real DTLS-SRTP/Opus media on a laptop — re-verified on every version bump, because losing it would silently take that test with it. Pinned at `151.0.0` until upstream deleted that release; see ADR-020 Amendment A1 and the availability risk in §12. [ADR-020](DECISIONS/ADR-020-webrtc-voice-foundation.md), [evidence](test-results/phase2a-webrtc-spike-20260828.md) |
...
| **The Apple WebRTC artifact's availability depends on a third party's GitHub release** | **High** | Any build, including CI | Fired on 2 Sep 2026, five days after being introduced: upstream deleted the `151.0.0` release and every build 404ed. Re-pinned to `152.0.0` and re-validated. A checksum protects integrity — it did, correctly — but not availability. Vendoring the ~45 MB XCFramework is the only mitigation that removes the failure mode; deferred, and to be reconsidered on the next occurrence. Maven Central forbids deleting a published artifact, so Android is not exposed. [ADR-020 Amendment A1](DECISIONS/ADR-020-webrtc-voice-foundation.md#amendment-a1--2-september-2026--the-apple-pin-moves-to-m152-because-upstream-deleted-the-m151-release), STATUS §4 problem 27 |
