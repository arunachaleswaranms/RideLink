...
|---|---|---|---|
| Self-signed X.509 identity generation on iOS | **High** | Phase 1b | `SecKey` gives no cert-building API; needs a small hand-written DER X.509 encoder (~150 lines, well-trodden). **There is no validated fallback** — if this proves infeasible, the response is a focused cross-platform secure-transport design review, not a substitute transport ([ADR-007 Amendment A1](DECISIONS/ADR-007-control-channel-over-tcp-tls.md#amendment-a1--26-august-2026--secure-transport-contingency)) |
| TLS keying-material **exporter** availability on both platforms | **High** | Phase 1b | The SAS depends on it ([PROTOCOL §4.5.1](PROTOCOL.md#451-the-six-digit-sas--exact-construction)). Neither platform's public exporter surface is confirmed. Same response as above: spike it early, and if it is absent, redesign the channel binding deliberately — do not weaken it |
