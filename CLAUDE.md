...
| Why this way? | `docs/DECISIONS/` (ADR-001…016) |
...
Reasoning: `docs/DECISIONS/ADR-001…016`.
...
Android `core` (JVM) and `RideLinkCore` (pure Swift) are mirror images. `android/` and `ios/`
**do not exist yet** — they are created during Phase 1 scaffolding.
...
- `vectors/sas/` and `vectors/identity/` contain fabricated test values only. Never a real key, token, exporter output or pairing code.
