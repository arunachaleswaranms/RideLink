...
| Why this way? | `docs/DECISIONS/` (ADR-001…027) |
...
| 26 | **One pure coexistence owner decides every intercom-caused music effect** (ADR-027). Ducking multiplies, and never rewrites, the user's base volume; Mode D is exact-track local suppression rather than a second synchronized-playback authority. Every ramp/pause/resume carries session and player ownership, teardown joins restoration, reconnect inherits only an outstanding restoration obligation, and one process-global owner writes `AVAudioSession`. **A continuous (gate-`none`) policy's permanently-enabled outbound track is never itself speech** (ADR-027 Amendment A1): `SpeechActivity` is the honest signal coexistence reduces on, genuinely separate from `transmitting`/`peerTransmitting` (which keep meaning "the track is enabled"), and a continuous gate reports it `.unavailable`, never a fabricated `.active`. **A voice diagnostics snapshot's provenance travels with it**: `VoiceDiagnostics.controlGeneration`, stamped once from `VoiceNegotiationState.negotiationControlGeneration`, is checked before coexistence ever applies a snapshot — never re-derived from whatever generation happens to be live when the snapshot is consumed | Never set music gain from voice/UI/audio-session callbacks; never send Phase 5 `PAUSE` for Mode D; never resume over user intent; never let an old completion touch a successor; never create a second player or audio-session writer; never read continuous transmission as speech; never seed a fresh coexistence generation from a cached diagnostics value instead of letting its own provenance-checked snapshots arrive |

| 27 | **Local capacity for an authoritative command is reserved before the command can be delivered, never refused after it** (ADR-024 Amendment A11, superseding ADR-029 decision 3). The bounded wire queues bound what is *on the wire*; they say nothing about the ordered apply node a drained frame leaves behind, or the scheduled node that node arms — so that work is bounded too, by `SessionWorkLedger`. But **where** the bound is enforced is the whole decision: a leader's apply node is created by the outbound commit hook, which runs *after* `send` returned true, so refusing there abandons authority the follower already has. Capacity is therefore taken at `issue` (inside the same critical section as the `command_seq` allocation and the enqueue), at `admitAuthoritativeCommand` before either sequence number moves, at the drain before the pop, and at `restoreFromPlaybackState` before its one `applyPlay` — and a refusal at each has an existing, reviewed answer: `failClosedOutbound` with **`SyncState.LOCAL_OVERLOAD`** (not `TRANSPORT_FAILED`: the transport was never asked), `latchDesynchronized` with **no `command_seq` spent**, leave the item held, or retain the snapshot as `DEFERRED_CAPACITY`. A `WorkReservation` is an immutable `(id, generation)` token from a never-reused counter, refcounted because one command's obligation spans an apply node and the scheduled node it arms, with **exactly one release site per reservation** in a `finally`/`defer`; `enterPhase` runs synchronously inside the phase already held, which is what makes the refcount structurally rather than carefully safe. `retire` is bounded by the generation that **ended**, so a retired session's cleanup can never free a successor's capacity | Never ask "is there capacity?" downstream of a successful send; never abandon, cancel or roll back a command the peer has; never clear `lastAppliedSeq`/`lastReceivedSeq` for anything short of a retired control lifetime; never fix it by raising the bound, dropping the oldest or newest node, or adding a wire message; never release a reservation by reading live state instead of handing back the token you were given; never release one twice — a live armed effect still owns it — and never let a replay mint a replacement reservation any more than a replacement `RideAdmission` |

| 28 | **Transport ownership has one source, and finishing old authority never reopens it** (ADR-024 Amendment A14). "May a fresh local transport control become synchronised authority?" is `syncEnabled && role != nil` and nothing else — set by an explicit activation (local Play-synced, an authoritative command accepted from the leader, the leader serving a follower's intent), cleared by every exit. Accepted or delivered debt finishing after End Ride legitimately reports SCHEDULED/SYNCED, and the role survives End Ride with the control connection, so **neither `SyncState` nor `role != nil` is evidence of ownership**. iOS's gate reads `SyncPlaybackCoordinator.transportOwnership`, a lock-backed mirror stored by the `didSet` of `syncEnabled` and `role`; Android's reads `isSynchronizedModeActive()`. Both coordinators also admit a fresh local press only through `admitLocalTransport()` and re-prove it in `issue(origin: .localTransport)` beside the stamp, because the gate's read is acted on later and the synchronised-playback cards call the entry points directly | Never derive ownership from `SyncState`, a display field or a published snapshot; never let debt completion, a nominal Start Ride or a surviving role activate it; never gate inbound, accepted, delivered or resync authority on it; never route a local music control (lock screen, `MediaSession`, in-app music, Ride Mode) around `MusicCoordinator`'s gate; never give `issue` a default origin |

Reasoning: `docs/DECISIONS/ADR-001…029`.
...
- **iOS:** one process-global `AVAudioSession` owner with **two** configurations — `.playback` for music-only, and `.playAndRecord`/`.voiceChat` with `[.allowBluetoothHFP, .allowBluetoothA2DP, .mixWithOthers]` for the intercom. `.allowBluetooth` is the deprecated spelling. RideLink applies its own player gain; it does not use `.duckOthers`. `UIBackgroundModes: audio`. Handle route change **and** interruption **and** media-services-reset.
...
python3 tools/generate_coexistence_vectors.py

# Cross-platform software integration gate — the Swift and Kotlin implementations as two
...
for it — the 24 September 2026 project audit, its fixes (STATUS problems 101–108) and the exact
...
right question **one step too late** — on a leader the apply node is created by the outbound commit
hook, which runs after `send` returned true, so refusing there abandoned authority the follower
already had. Capacity is now reserved before an authoritative command can be delivered
(ADR-024 Amendment A11). The second closed the cross-platform **software** gate with a live
two-implementation session over a real socket (`tools/crossplatform/run.sh`, ADR-029 Amendment A1) —
the first time the Kotlin and Swift implementations have ever spoken to each other, and the first
cross-platform confirmation that ADR-018's exporter-derived six digits agree. The interactive
emulator ↔ simulator UI journey is an **environment limitation**, recorded as one rather than folded
into the hardware-deferred list.
...
and no Android source file changed.

**A sixth independent review then audited round 6's own fixes and found one more confirmed blocker, in
three reachable forms, all fixed — and it is the first of these rounds to find the defect on *both*
platforms** ([ADR-028 Amendment A6](docs/DECISIONS/ADR-028-ride-mode-and-state-resynchronization.md#amendment-a6--20-september-2026--independent-review-round-7-retained-work-must-carry-the-ride-that-admitted-it);
STATUS §2ba, problem 91). **Its standing lesson: provenance that exists only while an operation is
*executing* is not provenance.** Round 6 was right that a ride lifetime must be captured at admission
and compared rather than re-read, and it threaded exactly that through every directly-executing apply
path. What it never asked is what happens when the operation stops executing and becomes **stored**:
at that moment its two threaded values went out of scope, `DeferredEvent` recorded the control
generation and the reconciliation obligation id and nothing else, and the replay — `drainDeferredEvents`,
after a clock recovered or a transfer finished — captured a **fresh** ride admission from whatever was
live by then. **Bug A**: a `PLAY` held for an untrustworthy clock under ride 1, replayed across an
accepted-but-uncleaned End Ride *and* a Start Ride, pre-rolled and started on the real player and was
stamped `rideAuthorityEpoch = 3` — ride 2's epoch on ride 1's work — after which ride 1's own
(correctly superseded-refusing) cleanup left it standing forever. **Bug B**: the same through a
retained `STATE_SNAPSHOT`, reported `RECONCILED` under ride 2 — the obligation id answers "is this S1
or S2?" and the generation answers "is this lifetime live?", and neither answers "is S1 still
authorised by the ride that admitted it?". **Bug C**: an **append-time** race, where the full-restore
pre-check suspends and then retains while re-proving only the control generation. Fixed with one
immutable `RideAdmission { synchronizedModeEpoch, rideEpoch }` captured at each path's **real**
admission point, **stored on the retained event**, replayed unchanged and proved before every write
and every retention; a required `ride:` parameter so a replay cannot silently mint a replacement; and
a drain that **pops** retired work, cancels its obligation (`REJECTED_RIDE` → `CANCELLED`, never
`RECONCILED`, never an indefinite deferral) and continues rather than wedging the stream.
`DeferredEvent.queueSnapshot` deliberately carries none — End Ride does not retire queue authority,
...
gate is also still open. At that historical point Phase 6 and Phase 7 had not started.**
...
- **Eight shared vector sets**, each an independent third transcription: `session-clock/`, `ordering/`, `drift/`, `queue/`, `playback-messages/`, `queue-messages/`, the audits' `phase5-gates/`, and Phase 6 `coexistence/`.
