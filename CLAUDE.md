...
**Phase 5 — synchronized playback. Software closure A4 complete; the real-device
...
complete on both platforms with their real-device gates open — see `docs/STATUS.md` §2q–§2ae.
...
four, then three. **Phase 5 has now been closure-audited four times** — A1 (ADR-024 Amendment A1):
...
A3*, which found four and then two more while building their regressions. Read all of that as the
standing lesson it is — on this codebase, "CI-green" and "correct" are different claims, and the gap
