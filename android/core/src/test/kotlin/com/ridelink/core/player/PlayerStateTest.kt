...
private val hash = ContentHash("sha256:" + "ab".repeat(32))
...
class PlayerStateTest {
    ...
    fun `reaching duration while playing has not ended yet`() {
        ...
        assertFalse(PlayerState(contentHash = hash, positionMs = 1000, durationMs = 1000, playing = true).ended)
    }

    @Test
    fun `stopped exactly at duration has ended`() {
        assertTrue(PlayerState(contentHash = hash, positionMs = 1000, durationMs = 1000, playing = false).ended)
    }

    @Test
    fun `a zero-length duration never reports ended`() {
        // Duration not yet known (still loading) must not look like "finished".
        assertFalse(PlayerState(contentHash = hash, positionMs = 0, durationMs = 0, playing = false).ended)
    }

    @Test
    fun `no loaded track never reports ended`() {
        assertFalse(PlayerState(contentHash = null, positionMs = 0, durationMs = 0, playing = false).ended)
    }
}
